locals {
  listener_port         = var.tcp_enabled && var.create_target_group ? (var.udp_enabled ? var.udp_port : var.tcp_port) : var.udp_port
...
resource "aws_lb_listener" "default" {
  count             = local.create_target_group  == true && var.tcp_enabled ? 1 : (local.create_target_group  == true && var.udp_enabled ? 1 : 0)
